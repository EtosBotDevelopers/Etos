...
def is_owner_check(message):
    return message.author.id == '188952933323243520' or message.author.id == '173547110388465664'
...
def role_or_permissions(ctx, check, **perms):
    if check_permissions(ctx, perms) or ctx.message.author.id == "188952933323243520" or ctx.message.author.id == "173547110388465664":
        return True
...
def mod_or_permissions(**perms):
    def predicate(ctx):
        return role_or_permissions(ctx, lambda r: r.name == 'Bot Manager', **perms)
